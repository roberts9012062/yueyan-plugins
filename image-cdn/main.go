...
// Cloudflare 图床插件（进程外，免费）：媒体上传直达 Cloudflare R2 对象存储。
//
// 架构（配对三件套）：
...
//
// 能力划分：
//   - settings：workers_url（Worker 地址）+ api_key（访问密钥）——保存后即时下发，
//     配对成功即接管上传（宿主 media.storage seam，见 internal/plugin/seam_media.go）
//   - api：GET /storage/health 配对探测、POST /storage/upload 转存（宿主 seam 契约端点）
//
// 上传流向：宿主 UploadMedia（类型/大小前置校验后）→ 本插件 /storage/upload →
// Worker POST /upload（Bearer Key）→ R2 → 公开 URL 回传落 media_assets。
package main
...
// ImageBedPlugin Cloudflare 图床插件实现（进程外）。
...
func (p *ImageBedPlugin) Info() sdk.Info {
	return sdk.Info{
		...
		Version:      "2.0.0",
		Author:       "月言官方",
		Description:  "媒体上传直达 Cloudflare R2 对象存储：填 Workers URL 与 Key 配对即用，公开 URL 回站展示。免费。",
		Capabilities: []string{"settings"},
		Settings: []sdk.SettingField{
			{Key: "workers_url", Label: "Workers URL（如 https://yueyan-image-bed.xxx.workers.dev）", Type: "text", Default: ""},
...
// RegisterAPI 契约端点（宿主 media.storage seam 适配器调用，System 身份）：
//
//	GET  /storage/health   配对探测 → {ok:true} 或 {ok:false,error}
...
// handleUpload 转存：解码内容 → 校验配对 → Worker /upload → 映射契约响应。
func handleUpload(ctx context.Context, method string, path string, body []byte) (int, []byte, error) {
	...
	result, err := uploadToWorker(cfg["workers_url"], cfg["api_key"], req.Filename, req.Mime, req.Content64)
	if err != nil {
