// nav-links/frontend/board.js
// 精品导航 · 通用导航看板（公开页 nav-page.js 与私有页 private-page.js 共用组件）：
//   顶部标题与搜索 → 三栏（左分类菜单 / 中导航卡片网格 / 右 3D 球形标签云）。
// 双主题：优先继承宿主 --yy-* 设计令牌；宿主变量缺失时按 prefers-color-scheme 兜底。
// 中栏双视图：列表式（默认）与九宫格，选择记忆在 localStorage。
// 数据注入式：调用方负责拉数与鉴权，本组件只做渲染与交互（createNavBoard → setData）。
...
// pageStyles 页面级样式（含响应式断点；随组件销毁移除）。
const pageStyles = `
...
export function createNavBoard(ctx, opts) {
  ...
  // iconHTML 图标（有图标用 img，否则首字母色块；纯函数）。
  ...
  const render = () => {
    ...
    box.innerHTML =
      ...
      (state.loaded && rows.length === 0
        ? '<p style="text-align:center;padding:40px 0;font-size:13px;color:var(--nl-text2)">' + (state.links.length === 0 ? "还没有收藏站点" : "没有符合筛选条件的站点") + "</p>"
        : "") +
      "</main>" +
      // 右栏:3D 标签云
  ...
  // setData 注入数据并重渲染（loaded 标记控制空态文案出现时机）。
  const setData = (data) => {
