...
import { createTagSphere } from "./tag-sphere.js?v=3";
...
export default function registerPage(ctx) {
  ...
  const cardHTML = (l) => {
    ...
    return (
      '<a href="' + escapeHtml(l.url) + '" target="' + (state.settings.open_new_tab !== "off" ? "_blank" : "_self") + '" rel="noopener noreferrer" class="nl-card">' +
      icon +
  ...
  const render = () => {
    ...
    box.innerHTML =
      ...
      "</div>" +
      '<div class="nl-grid" data-grid>' + rows.map(cardHTML).join("") + "</div>" +
